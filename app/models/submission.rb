class Submission < ApplicationRecord
  ...
  scope :trending, -> {
    joins(:user_submissions)
      ...
      .order(Arel.sql("COUNT(user_submissions.id) DESC, submissions.created_at DESC"))
  }
  ...
  scope :new_hot, -> {
    where("submissions.created_at >= ?", 7.days.ago)
      ...
      .group("submissions.id")
      .order(Arel.sql("COALESCE(SUM(CASE WHEN user_submissions.upvote = true THEN 1 ELSE 0 END), 0) DESC, submissions.created_at DESC"))
  }
