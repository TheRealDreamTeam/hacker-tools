# Configure Redis SSL for Heroku Redis (rediss:// URLs)
...
#
# This initializer configures Redis connections globally to handle SSL properly
# when connecting to Heroku Redis. It applies to:
# - Action Cable (Turbo Streams)
# - Active Job (if using Redis adapter)
# - Any other Redis connections in the application
#
# We override the Redis#initialize method to automatically add SSL configuration
...
if Rails.env.production? && ENV["REDIS_URL"].present?
  ...
  if redis_url.scheme == "rediss"
    ...
    # Override Redis#initialize to add SSL configuration for rediss:// URLs
